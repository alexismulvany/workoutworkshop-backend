...
-- tracks changes in user weight over time
...
create table exercises(
                          ...
                          muscle_group ENUM('Back', 'Arms', 'Legs', 'Chest', 'Core', 'Cardio'), --  muscle group that workout targets
...
-- holds fully planned workout
create table workout_plans(
                              plan_id integer auto_increment not null,
                              user_id integer not null, -- user workout is intended for
                              title varchar(16) not null,
                              planned_date varchar(10) not null, -- MM-dd-yyyy
                              create_date datetime default CURRENT_TIMESTAMP,
                              last_update datetime default CURRENT_TIMESTAMP,
                              primary key(plan_id),
                              foreign key (user_id) references users(user_id)
...
create table plan_exercise(
                              ...
                              weight integer DEFAULT 135,
                              completed boolean default false, -- track if exercise has been completed
